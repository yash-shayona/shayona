    ...
    for t in tasks:
        ...
        rbs_task_updates = frappe.get_all(
            ...
            fields=[
                ...
                "task_status",
                "new_static_product",
                "existing_static_product",
                "new_apparel_product",
                "existing_apparel_product",
                "new_variable_product",
                "existing_variable_product",
                "new_category",
                "existing_category",
                "revised_product",
                "revised_category",
            ...
            filters={
                "parent": t.name,
                "parenttype": "Task",
                **filters["rbs_task_update_filters"],
            },
        ...
        for idx, rbstu in enumerate(rbs_task_updates):
            if idx == 0:
                ...
                task_name = t.name
                subject = t.subject
                custom_task_owner = t.custom_task_owner
...
def products_cell_prepare(rbstu_row):
    ...
    if (
        rbstu_row.get("existing_static_product")
        and rbstu_row.get("existing_static_product") > 0
    ):
        products.append(
            "Existing Static: " + str(rbstu_row.get("existing_static_product"))
        )
    if (
        rbstu_row.get("new_apparel_product")
        and rbstu_row.get("new_apparel_product") > 0
    ):
        products.append("New Apparel: " + str(rbstu_row.get("new_apparel_product")))
    if (
        rbstu_row.get("existing_apparel_product")
        and rbstu_row.get("existing_apparel_product") > 0
    ):
        products.append(
            "Existing Apparel: " + str(rbstu_row.get("existing_apparel_product"))
        )
    if (
        rbstu_row.get("new_variable_product")
        and rbstu_row.get("new_variable_product") > 0
    ):
        products.append("New Variable: " + str(rbstu_row.get("new_variable_product")))
    if (
        rbstu_row.get("existing_variable_product")
        and rbstu_row.get("existing_variable_product") > 0
    ):
        products.append(
            "Existing Variable: " + str(rbstu_row.get("existing_variable_product"))
        )
