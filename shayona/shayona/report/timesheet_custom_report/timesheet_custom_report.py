...
def get_data(filters) -> list[list]:
	...
	timesheet = frappe.get_all(
     "Timesheet", 
     filters=filters, 
     fields=["name", "employee", "employee_name", "start_date", "end_date", "custom_day", "total_hours", "custom_total_break_hours", "status", "custom_auto_submit"],
     order_by="name desc"
     )
	
	for ts in timesheet:
		logs = frappe.get_all("Timesheet Detail",
		...
		if logs:
			...
			entry_time = format_time(entry_dt, format_string="hh:mm:ss a") if entry_dt else None
			exit_time  = format_time(exit_dt, format_string="hh:mm:ss a") if exit_dt else None
		else:
